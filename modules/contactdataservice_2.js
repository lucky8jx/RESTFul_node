	...
	model.findOne({primarycontactnumber: primarynumber}, function(error, data) {
		if (error) {
			...
			data.save(function (error) {
				if (!error) {
					console.log('Successfully updated contact with primarynumber: '+ primarynumber);
					data.save();
				} else {
...
exports.paginate = function(model, request, response) {
	console.log(model.paginate.toString());
	model.paginate({}, request.query.page, request.query.limit, function(error, pageCount, result, itemCount) {
		if (error) {
			console.log(error);
			response.writeHead(500, {'Content-Type' : 'text/plain'});
			response.end('Internal server error');
	...
	// model.paginate({}, request.query.page, request.query.limit, callback);
};
